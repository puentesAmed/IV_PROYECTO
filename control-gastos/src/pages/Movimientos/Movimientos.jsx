...
export function Movimientos() {
  const { list, loading, hasMore, filters, setFilter, loadNextPage, remove } =
    useMovimientos({
      q: '',
      categoria: '',
      limit: 20
  ...
  const columns = useMemo(
    () => [
      ...
      { key: 'categoria', header: 'Categoría' },
      {
        ...
        header: 'Importe',
        render: (r) =>
          eur.format(Number.isFinite(+r.importe) ? +r.importe : 0),
        align: 'right'
      },
      {
        key: 'acciones',
        ...
        render: (r) => (
          <button
            ...
            aria-label={`Eliminar movimiento ${r.concepto ?? r.id}`}
            title="Eliminar"
          >
            <img
              src={eliminar}
              alt=""
              className="icon-invert-white"
            />
          </button>
        )
      }
  ...
  return (
    <section className="movimientos-view">
      <header className="mov-head">
        <h1>Movimientos</h1>
        <p className="muted">
          Consulta, busca y gestiona todos tus movimientos.
        </p>
      </header>
      ...
      >
        ...
        {loading && (
          <p role="status" style={{ textAlign: 'center', margin: '8px 0' }}>
            Cargando…
          </p>
        )}
