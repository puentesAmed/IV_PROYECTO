...
const addMonths = (y, m, delta) => { const d = new Date(y, m, 1); d.setMonth(d.getMonth()+delta); d.setHours(0,0,0,0); return d; };
const yyyymm = (d) => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;

const sumTotals = (series) => {
  const t = series.reduce((a,r)=>({ ingresos:a.ingresos+r.ingresos, gastos:a.gastos+r.gastos }), { ingresos:0, gastos:0 });
...
export function aggregateByDates(list, dateStart, dateEnd) {
  ...
  for (let d = new Date(s); d <= e; d = addDays(d, 1)) {
    series.push({ key:+d, label: d.toLocaleDateString('es-ES', { day:'2-digit', month:'2-digit' }), ingresos:0, gastos:0 });
  }
...
export function aggregateByYears(list, yearStart, yearEnd) {
  const ys = Number(yearStart), ye = Number(yearEnd);
  const s = Math.min(ys, ye), e = Math.max(ys, ye);

  const series = [];
  for (let y = s; y <= e; y++) {
    series.push({ key:String(y), label:String(y), ingresos:0, gastos:0 });
  }
  const map = new Map(series.map(r => [r.key, r]));
...
export function aggregate(list, mode, ranges){
  if (mode === 'dates')  return aggregateByDates(list, ranges.dateStart, ranges.dateEnd);
  if (mode === 'months') return aggregateByMonths(list, ranges.monthStart, ranges.monthEnd);
