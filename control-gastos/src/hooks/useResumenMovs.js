...
export function useResumenMovs(list = [], {
  ...
  const bounds = useMemo(() => {
    let s, e;
    if (mode === 'dates') {
      s = new Date(dateStart); e = new Date(dateEnd);
    } else if (mode === 'months') {
  ...
  const catRows = useMemo(() => {
    ...
    filtered.forEach(m => {
      ...
      const row = map.get(key) || { categoria: key, ingresos: 0, gastos: 0, balance: 0 };
      const val = Number(m.importe) || 0;
      if (m.categoria === 'Ingresos') row.ingresos += Math.max(0, val);
      else row.gastos += Math.abs(val);
      map.set(key, row);
    });
    const rows = Array.from(map.values()).map(r => ({ ...r, balance: r.ingresos - r.gastos }));
