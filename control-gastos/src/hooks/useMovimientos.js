...
const DEFAULTS = { page: 1, limit: 'ALL', q: '', categoria: '' }; // <- TODOS por defecto
...
export function useMovimientos(initial = {}) {
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [hasMore, setHasMore] = useState(false); // en “ALL” no hay más
  ...
  useEffect(() => { // cada cambio de filtro trae todo de nuevo
    setList([]);
    fetchMovimientos({ page: 1, append: false });
  }, [filters.q, filters.categoria, filters.limit, fetchMovimientos]);

  const loadNextPage = useCallback(() => {
    if (String(filters.limit).toUpperCase() === 'ALL') return; // sin paginación
    if (loading || !hasMore) return;
